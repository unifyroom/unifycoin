...
The Unifyroom Core daemon can be configured to act as a trusted "border
router", implementing the unfy wire protocol and relay, making
...
The ZeroMQ feature in Unifyroom Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building unfyd:
...
For instance:
    ...
    $ unfyd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/unfyd.tx.raw \
...
These options can also be provided in unfy.conf.
...
From the perspective of unfyd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into unfyd directly. Furthermore, no information is
...
using. Unifyroomd appends an up-counting sequence number to each
