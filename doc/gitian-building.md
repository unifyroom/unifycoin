...
*Setup instructions for a Gitian build of Unifyroom Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Unifyroom
...
- [Building Unifyroom Core](#building-unfy-core)
...
Clone the git repositories for Unifyroom Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/unifyroom/unifycoin
git clone https://github.com/unifyroom/gitian.sigs.git
```
...
in the Unifyroom Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Unifyroom Core
----------------

To build Unifyroom Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Unifyroom Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/unfy/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/unifyroom/unifycoin
...
For example:
```bash
URL=https://github.com/crowning-/unfy.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit unfy=${COMMIT} --url unfy=${URL} ../unfy/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit unfy=${COMMIT} --url unfy=${URL} ../unfy/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit unfy=${COMMIT} --url unfy=${URL} ../unfy/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the unfy git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../unfy/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/unfy
SIGPATH=/some/root/path/unfy-detached-sigs

./bin/gbuild --url unfy=${BTCPATH},signature=${SIGPATH} ../unfy/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/unfy-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/unfy-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/unfy-osx-build.assert
```
...
[unfy/gitian.sigs](https://github.com/unifyroom/gitian.sigs/) repository, or if that's not possible to create a pull
request.
