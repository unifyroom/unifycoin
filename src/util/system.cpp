...
const char * const BITCOIN_CONF_FILENAME = "unfy.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //unfy-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\UnifyroomCore
    // macOS: ~/Library/Application Support/UnifyroomCore
    // Unix-like: ~/.unfycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UnifyroomCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/UnifyroomCore";
#else
    // Unix-like
    return pathRet / ".unfycore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty unfy.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure Unifyroom Core copyright is not removed by accident
    if (copyright_devs.find("Unifyroom Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Unifyroom Core developers";
    }
