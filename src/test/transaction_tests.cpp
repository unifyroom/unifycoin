...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            BOOST_CHECK_MESSAGE(CheckTransactionD(tx, state), strTest);
...
BOOST_AUTO_TEST_CASE(tx_invalid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            TxValidationState state;
            fValid = CheckTransactionD(tx, state) && state.IsValid();
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    stream >> tx;
    TxValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransactionD(CTransaction(tx), state) && state.IsValid(), "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransactionD(CTransaction(tx), state) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}
