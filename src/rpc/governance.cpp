...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 UNFY irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on unfycentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Unifyroom features */
    { "unfy",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "unfy",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "unfy",               "gobject",                &gobject,                {} },
    { "unfy",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
