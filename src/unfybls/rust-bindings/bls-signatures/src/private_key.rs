...
impl PrivateKey {
    ...
    #[cfg(feature = "unfy_helpers")]
    pub fn generate_unfy(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "unfy_helpers")]
    pub fn generate_unfy_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_unfy(rng))
            .collect()
    }
