...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckTransaction(tx, state, 5, nModifiedFees))
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in Unifyroom
...
static int64_t nTimeUnifyroomSpecific = 0;
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(pindex, block, state, m_params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, m_mnhfManager, *m_quorum_block_processor, *m_clhandler, m_params.GetConsensus(), view, fJustCheck, fScriptChecks, state, mnlist_updates_opt)) {
        return error("ConnectBlock(UNFY): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // UNFY : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(UNFY): chain-locked transaction %s overrides islock %s\n",
                    ...
                    LogPrintf("ERROR: ConnectBlock(UNFY): transaction %s conflicts with transaction lock %s\n", tx->GetHash().ToString(), conflictLock->txid.ToString());
    ...
    // UNFY : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!CheckCreditPoolDiffForBlock(block, pindex, m_params.GetConsensus(), blockSubsidy, state)) {
        return error("ConnectBlock(UNFY): CheckCreditPoolDiffForBlock for block %s failed with %s",
                     pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (!MasternodePayments::IsBlockValueValid(*sporkManager, *governance, *::masternodeSync, block, pindex->nHeight, blockSubsidy + feeReward, strError)) {
        ...
        LogPrintf("ERROR: ConnectBlock(UNFY): %s\n", strError);
    ...
    if (!MasternodePayments::IsBlockPayeeValid(*sporkManager, *governance, *::masternodeSync, *block.vtx[0], pindex->pprev, blockSubsidy, feeReward)) {
        // NOTE: Do not punish, the node might be missing governance data
        LogPrintf("ERROR: ConnectBlock(UNFY): couldn't find masternode or superblock payments\n");
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeUnifyroomSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Unifyroom specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeUnifyroomSpecific * MICRO, nTimeUnifyroomSpecific * MILLI / nBlocksTotal);
...
bool CheckBlock(const CBlockIndex* pindexPrev, const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{   
    ...
    for (const auto& tx : block.vtx) {
        ...
        if(isGenesis){
            if (!CheckTransactionD(*tx, tx_state)) {
            ...
            if (!CheckTransaction(*tx, tx_state, nHeight - 1, blockRewardk)) {
                // CheckBlock() does context-free validation checks. The only
                // possible failures are consensus failures.
                assert(tx_state.GetResult() == TxValidationResult::TX_CONSENSUS);
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, tx_state.GetRejectReason(),
...
bool CheckBlockD(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (!CheckBlock(pindex, block, state, m_params.GetConsensus()) ||
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlock(pindexPrev, block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, state.ToString());
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(pindex, block, state, chainparams.GetConsensus()))
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, m_mnhfManager, *m_quorum_block_processor, *m_clhandler, m_params.GetConsensus(), inputs, false /*fJustCheck*/, false /*fScriptChecks*/, state, mnlist_updates_opt)) {
        return error("RollforwardBlock(UNFY): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (fAddressIndex) {
        if (!pblocktree->WriteAddressIndex(addressIndex)) {
            return error("RollforwardBlock(UNFY): Failed to write address index");
        }

        if (!pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("RollforwardBlock(UNFY): Failed to write address unspent index");
        }
    }

    if (fSpentIndex) {
        if (!pblocktree->UpdateSpentIndex(spentIndex))
            return error("RollforwardBlock(UNFY): Failed to write transaction index");
    }

    if (fTimestampIndex) {
        if (!pblocktree->WriteTimestampIndex(CTimestampIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return error("RollforwardBlock(UNFY): Failed to write timestamp index");
    }
...
bool CChainState::ReplayBlocks()
{
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        if (fDIP0003Active && !m_evoDb.VerifyBestBlock(pindexOld->GetBlockHash())) {
            return error("ReplayBlocks(UNFY): Found EvoDB inconsistency");
        }
    }
...
bool CChainState::LoadGenesisBlock()
{
    ...
    try {
        ...
        if (m_params.NetworkIDString() == CBaseChainParams::DEVNET) {
            ...
            // bool fCheckBlock = CheckBlock(pindex, *shared_pblock, state, m_params.GetConsensus());
            // assert(fCheckBlock);
